package com.nttdata.transactioncurrent.controller;
...
public class TransactionController {
  ...
  public Mono<ResponseEntity<Transaction>> create(
    ...
    return service.countMovements(transaction.getCurrentAccount().getId()) // N°
                                                                           // Movimientos
                                                                           // actuales
        .flatMap(cnt -> {
          return service
              .findSavingAccountById(transaction.getCurrentAccount().getId()) // Busco
                                                                              // la
                                                                              // Cuenta
                                                                              // Bancaria
              .flatMap(ca -> {
                switch (transaction.getTypeTransaction()) {
                  case DEPOSIT :
                    ca.setBalance(
                        ca.getBalance() + transaction.getTransactionAmount());
                    break;
                  case DRAFT :
                    ca.setBalance(
                        ca.getBalance() - transaction.getTransactionAmount());
                ...
                if (cnt >= ca.getFreeTransactions()) {
                  ca.setBalance(
                      ca.getBalance() - ca.getCommissionTransactions());
                  transaction
                      .setCommissionAmount(ca.getCommissionTransactions());
                } else {
                  transaction.setCommissionAmount(0.0);
                }
                return service.updateSavingAccount(ca).flatMap(saveCa -> {
  ...
  public Mono<ResponseEntity<Transaction>> update(
    ...
    return service.findById(transaction.getId())
        .flatMap(tcaDB -> service
            .findSavingAccountById(transaction.getCurrentAccount().getId())
            .flatMap(ca -> {
              switch (transaction.getTypeTransaction()) {
                case DEPOSIT :
                  ca.setBalance(ca.getBalance() - tcaDB.getTransactionAmount()
